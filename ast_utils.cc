...
ASTUtils::mk_varDecl_node(ASTNode* n1, string* id1){
	...
	if(vdata != NULL && vdata->get_m() == stab->method){
...
ASTUtils::mk_methDecl_node(ASTNode* n1, string* id1,ASTNode* n2, ASTNode* n3, ASTNode* n4, ASTNode* n5){
	...
	TabData* vdata = stab->lookup(*id1);
	ASTMethodDeclNode* node = new ASTMethodDeclNode(n1, id1, n2, n3, n4, n5);
	if (vdata != NULL && vdata->is_method()){
		cout << "Already defined method ";
		cout << *id1 << ". Line " << line_num << endl;
		ASTMethodDeclNode* nodeT = (ASTMethodDeclNode*) vdata->get_node();
		nodeT->init(n1, id1, n2, n3, n4, n5);
		nodeT->set_lineno(line_num);
		deleteTemp(id1);
		return nodeT;
	}
	else{
...
ASTUtils::mk_stEq_node(string* name1, ASTNode* n2){
	...
	if(vdata== NULL){
		...
		ASTNode* node2 = new ASTVarDeclNode(new ASTTypeNode(&inteiro), name1);
		node2->set_lineno(line_num);
		vdata->set_node(node2);
...
ASTUtils::mk_expThis_node(){
	...
	if (tab == NULL){
		...
		node = new ASTExpThisNode(stab->globalClass);
		varsok = false;
	}
	else{
		cout << "THIS . . . . . . " << tab->get_name() << endl;
		node = new ASTExpThisNode(stab->globalClass);
	}
	node->set_lineno(line_num);
	node->setST(stab);
...
ASTNode*
ASTUtils::mk_expOperatorL_node(ASTNode* n1, string* op, ASTNode* n2){
	assert(n1!=NULL && op!=NULL && n2!=NULL);
	ASTExpOperatorLNode* node = new ASTExpOperatorLNode(n1,op,n2);
	node->set_lineno(line_num);
	return node;
}
...
ASTUtils::mk_expArray_node(ASTNode* n1, ASTNode* n2){
	...
	node->set_lineno(line_num);
	cout << "Making exp array with type " << *node->get_type() << " in line "<<line_num<<endl;
	return node;
}
...
ASTUtils::mk_newint_array_node(ASTNode* n1){
	...
	node->set_lineno(line_num);
	cout << "Making int array with type " << *node->get_type() << " in line "<<line_num<<endl;
	return node;
}
